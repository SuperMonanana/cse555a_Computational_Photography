...
if DO_MIXED
    % read images
    im_background = imresize(im2double(imread('./samples/im3.jpg')), 0.15, 'bilinear');
    im_s = imresize(im2double(imread('./samples/penguin.jpg')), 0.25, 'bilinear');

    % get source region mask from the user
    objmask = getMask(im_s);
    % align im_s and mask_s with im_background
    [im_s, mask_s] = alignSource(im_s, objmask, im_background);
